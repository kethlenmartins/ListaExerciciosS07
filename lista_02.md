...
var NUMERICO idade
var BOOLEAN podeVotar
var BOOLEAN obrigadoAVotar
...
ou se (16 < idade < 18) {
    podeVotar = true;
    obrigadoAVotar = false;
    escreva ("Você já pode votar, mas seu voto é facultativo!")
}
fim se
...
Classe Retangulo herda FormaGeometrica:
    Método Construtor (cor, largura, comprimento, area){
        puxar (cor);
        - largura
        - comprimento
        - area
    ...
    Método CalcularArea (largura, comprimento, area){
        area = largura * comprimento;
        escreva ("A área do retângulo é" + area)
    }

Classe Circulo herda FormaGeometrica:
    Método Construtor (cor, raio, area) {
        puxar (cor);
        - raio
        - area
    }

    Método CalcularArea (raio, area) {
        area = π * raio^2
...
```
```
var NUMERICO velocidadeCarro
var NUMERICO velocidadeInicial = 0
var NUMERICO tempo
var NUMERICO aceleracao
var NUMERICO velocidadeMaxima
var NUMERICO tempoMaximo
var NUMERICO distanciaPercorrida
var NUMERICO distanciaTotal

INICIO

for (tempo = 0; tempo < tempoMaximo; tempo++) {
    enquanto (velocidadeCarro < velocidadeMaxima & distanciaPercorrida < distanciaTotal) {
        velocidadeCarro += aceleracao*tempo
    }
    fim enquanto
}
fim for

função calcularTempo () {
    distanciaTotal = velocidadeInicial * tempoMaximo + (aceleracao%2 * tempoMaximo^2);
    escreva (tempoMaximo);
}

se (velocidadeCarro = velocidadeMaxima) {
    escreva ("Velocidade máxima atingida, não ultrapasse para evitar acidentes!");
    velocidadeCarro = velocidadeMaxima;
}
fim se

se (tempo = tempoMaximo) {
    escreva ("O tempo terminou!");
    velocidadeCarro = 0;
}
fim se

se (distanciaPercorrida = distanciaTotal) {
    escreva ("Corrida terminada.");
    velocidadeCarro = 0;
}
...
Função MultiplicaçãoDeMatrizes(matrizA, matrizB):
    ...
    se (colunasA.comprimento != linhasB.comprimento) {
        escreva ("As matrizes não atendem à condição para serem multiplicadas.")
    }
    senão {
        escreva ("As matrizes atendem à condição e podem ser multiplicadas")
    }
    fim se

    matrizResultado <- linhaA.comprimento, colunaB.comprimento
    ...
    Para i de 0 até linhas - 1, faça:
        para j de 0 até colunas - 1, faça:
        matrizResultado <- [(linhaA[1]*colunaB[1]) + (linhaA[1]*colunaB[2]) + (linhaA[2]*colunaB[1]) + (linhaA[2]*colunaB[2]) + ... + (linhaA[linha - 1]*colunaB[coluna - 1])]
